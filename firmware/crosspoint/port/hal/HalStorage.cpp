/* WODLE-PORT: HalStorage over RT-Thread DFS (elm-FAT on spi_msd "sd0").
 * Mirrors the upstream SdFat-backed API; paths are absolute on the mounted
 * card root ("/"), matching upstream usage (/.crosspoint, /books, ...). */
...
static const char *TAG = "HalStorage";
...
public:
    int fd = -1;
    DIR *dir = nullptr;
    std::string path; /* absolute path (for getName/rename/openNextFile) */
...
{
public:
    explicit StorageLock(SemaphoreHandle_t m) : _m(m)
    {
...
bool HalStorage::begin()
{
    ...
    /* spi_msd auto-init attaches "sdcard" on spi1 and registers "sd0";
     * poll briefly in case card detection is still settling. */
    rt_device_t dev = RT_NULL;
    for (int i = 0; i < 100 && !(dev = rt_device_find("sd0")); i++)
        rt_thread_mdelay(30);
    if (!dev)
    {
        LOG_ERR(TAG, "sd0 not found (no card / msd init failed)");
        return false;
    }

    if (dfs_mount("sd0", "/", "elm", 0, 0) != 0)
    {
        LOG_ERR(TAG, "dfs_mount sd0 on / failed (errno %d)", rt_get_errno());
        return false;
    }
    LOG_INF(TAG, "SD mounted on /");
    initialized = true;
    return true;
}
...
bool HalStorage::writeFile(const char *path, const String &content)
{
    ...
    int n = ::write(fd, content.c_str(), len);
    ::close(fd);
    return n == (int)len;
}
...
bool HalStorage::mkdir(const char *path, const bool pFlag)
{
    ...
    if (!pFlag) return ::mkdir(path, 0) == 0;
    ...
    for (size_t i = 1; i < p.size(); i++)
    {
        ...
        {
            p[i] = '\0';
            if (::stat(p.c_str(), &st) != 0) ::mkdir(p.c_str(), 0);
            p[i] = '/';
        }
    }
    return ::mkdir(p.c_str(), 0) == 0 || (::stat(path, &st) == 0 && S_ISDIR(st.st_mode));
}
...
static bool openChecked(const char *moduleName, const char *path, oflag_t flags, HalFile &file)
{
    file = Storage.open(path, flags);
    if (!file)
    {
        LOG_ERR(moduleName, "open failed: %s", path);
        return false;
    }
    return true;
}
