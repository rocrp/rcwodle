...
```bash
source ~/w/_tmp/SiFli-SDK/export.sh          # SIFLI_SDK + arm-none-eabi-gcc + sftool
...
## Flash (both region-safe — they only write the app region; the stock bootloader stays)

**SD `tf_ota`** (proven, primary):
```bash
APP=$(ls build_wodle_hcpu/output/main.bin)
...
# copy /tmp/wodle_fw/{hcpu_app.bin,update.json} → recovery SD card /firmware/, insert, power on
```
tf_ota only flashes a **newer** version than installed; bump `--version` each time.

**HVR1 USB** (faster once proven E2E): enter recovery, then
```bash
uv run ../../../tools/wodle_flash.py write build_wodle_hcpu/output/main.bin --addr 0x12218000
```

**Restore to stock** if a build doesn't boot: copy `refs/card_snapshot_0246/firmware/*` back to the
card `/firmware/` (keep that snapshot as the known-good un-brick).
